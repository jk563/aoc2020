...
fn main() {
    ...
    for line in lines {
        ...
        for child in line_components[1].trim().split(",").collect::<Vec<&str>>() {
            ...
            node_parents.entry(bag_type.clone()).or_insert(HashSet::new());
            node_parents.get_mut(&bag_type).unwrap().insert(parent_bag_type.clone());
    ...
    while sized_bags.len() < node_parents.len() || ! sized_bags.contains_key("shiny gold") {
        for (bag_type, bag_children) in &node_children {
            if sized_bags.contains_key(bag_type) { continue;}
            if node_children.get(bag_type).unwrap().len() == 0 {
                sized_bags.insert(bag_type.clone(), 1);
                continue;
            }
            if bag_children.len() == bag_children.keys().filter(|x| sized_bags.contains_key(&x.to_string())).count() {
                sized_bags.insert(bag_type.clone(), bag_children.iter().map(|(key, value)| sized_bags.get(key).unwrap() * value).sum::<usize>() + 1);
            }
        }
    }
    
    println!("Bag colours containing shiny gold: {}", contained_by("shiny gold", &node_parents).len());
    println!("Bags required in shiny gold: {}", sized_bags.get("shiny gold").unwrap() - 1);
}
...
fn contained_by(bag_type: &str, node_parents: &HashMap<String, HashSet<String>>) -> Vec<String>{
    ...
    while parents.len() != 0 {
        ...
        if node_parents.contains_key(parent) {
            parents.append(&mut node_parents.get(parent).unwrap().into_iter().collect::<Vec<&String>>());
        }
    }
    result_set.into_iter().map(|x| x.to_string()).collect::<Vec<String>>()
}
